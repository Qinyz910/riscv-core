...
### Instruction Memory
- **Protocol**: Simple request/response handshake with optional burst capability.
- **Timing**: Requests acknowledged with `instr_gnt_i`; responses may be delayed arbitrarily but are ordered.
- **Error Handling**: `instr_err_i` (optional) signals fetch faults, causing flush and trap to the exception handler.

### Data Memory
- **Protocol**: Mirrored after AXI-lite; supports single outstanding read and write transactions.
- **Byte Enables**: `data_be_o` allows sub-word accesses; invalid combinations generate exceptions.
- **Exceptions**: Address misalignment or access faults raise signals that update `mcause` and trigger a trap.
